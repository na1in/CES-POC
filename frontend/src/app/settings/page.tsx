...
export default function SettingsPage() {
  ...
  const { data, error, isLoading } = useSWR<ThresholdsResponse>("/api/settings/thresholds")
  ...
  if (error instanceof ApiError && error.status === 403) {
    showToast({ title: "Permission denied", type: "warning" })
  }
    ...
    <div style={{ minHeight: "100vh", background: "var(--pw-bg)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: "var(--pw-surface)", borderBottom: "1px solid var(--pw-border)", padding: "16px 24px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <h1 style={{ fontSize: 20, fontWeight: 700, fontFamily: "var(--pw-font-display)", color: "var(--pw-text-primary)", margin: 0 }}>Settings</h1>
        </div>
      ...
      <div style={{ flex: 1, padding: "24px", maxWidth: 960, width: "100%", boxSizing: "border-box" }}>
        ...
        {!isAdmin && (
          <p aria-label="Admin notice" style={{ fontSize: 13, color: "var(--pw-text-secondary)", margin: "0 0 16px" }}>
            ...
            <thead>
              <tr style={{ background: "var(--pw-bg)", borderBottom: "1px solid var(--pw-border)" }}>
                {["PARAMETER", "VALUE", "DESCRIPTION", "LAST CHANGED", ...(isAdmin ? ["ACTION"] : [])].map(col => (
                  <th
                    key={col}
                    style={{ padding: "10px 16px", fontSize: 10, fontWeight: 700, color: "var(--pw-text-muted)", textTransform: "uppercase", letterSpacing: "0.06em", textAlign: "left", whiteSpace: "nowrap" }}
                  >
                    {col}
                  </th>
                ))}
            ...
            <tbody>
              {isLoading
                ? Array.from({ length: 8 }, (_: unknown, i: number) => (
                    <tr key={i} style={{ borderBottom: "1px solid var(--pw-border)" }}>
                      {Array.from({ length: isAdmin ? 5 : 4 }, (_2: unknown, j: number) => (
                ...
                : error
                  ? (
                    <tr>
                      <td colSpan={isAdmin ? 5 : 4} style={{ padding: "24px 16px", textAlign: "center", color: "var(--pw-escalate)", fontSize: 13 }}>
                  ...
                  : thresholds.map((t, i) => (
                      <tr
                        ...
                        style={{ borderBottom: i < thresholds.length - 1 ? "1px solid var(--pw-border)" : "none", background: "transparent" }}
                      >
                        <td style={{ padding: "12px 16px", verticalAlign: "middle" }}>
                          <span style={{ fontFamily: "var(--pw-font-mono)", fontSize: 12, color: "var(--pw-text-primary)", background: "var(--pw-surface-elevated)", padding: "2px 6px", borderRadius: 4 }}>
                            {t.parameter_name}
                          </span>
                        </td>
                        <td style={{ padding: "12px 16px", verticalAlign: "middle" }}>
                          <span style={{ fontFamily: "var(--pw-font-mono)", fontSize: 14, fontWeight: 700, color: "var(--pw-primary)" }}>
                            {t.parameter_value}
                          </span>
                        </td>
                        ...
                        {isAdmin && (
                          <td style={{ padding: "12px 16px", verticalAlign: "middle" }}>
                            <button
                              ...
                              style={{ background: "none", border: "1px solid var(--pw-border)", borderRadius: 6, padding: "4px 10px", fontSize: 12, color: "var(--pw-primary)", cursor: "pointer", fontWeight: 500, whiteSpace: "nowrap" }}
                            >
                              Propose Change
                            </button>
                          </td>
        ...
        <p style={{ fontSize: 12, color: "var(--pw-text-muted)", marginTop: 14 }}>
          {isLoading ? "Loading…" : `${thresholds.length} thresholds · Changes require admin approval and are versioned in the audit log.`}
        </p>
      </div>
