...
describe("Queue Dashboard — default sort", () => {
  it("sorts rows by confidence score ascending (lowest first)", () => {
    ...
    // PMT-005 has confidence 4 — lowest — should be first data row
    expect(within(rows[0]).getByText("Unknown Remitter")).toBeInTheDocument()
    // PMT-001 has confidence 98 — highest — should be last row
    const lastRow = rows[rows.length - 1]
    // PMT-001 and PMT-006 both say "Riverside Medical Group"; check the last has $2,450.00
    expect(within(lastRow).getByText("$2,450.00")).toBeInTheDocument()
  })
})
...
describe("Queue Dashboard — navigation", () => {
  it("clicking a row calls router.push with the correct payment detail URL", () => {
    render(<QueueDashboard />)
    const rows = screen.getAllByRole("row").slice(1)
    // First row is PMT-005 (lowest confidence = 4)
  ...
  it("each payment routes to its own URL", () => {
    ...
    // Sort payments by confidence to match rendered order
